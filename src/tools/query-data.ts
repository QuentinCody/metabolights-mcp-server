...
interface QueryEnv {
    METABOLIGHTS_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("METABOLIGHTS_DATA_DO", "metabolights");

    server.registerTool(
        "metabolights_query_data",
        {
            title: "Query Staged MetaboLights Data",
            description:
                "Query staged MetaboLights data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
